...
class AI(object):
    ...
    def __update_danger(self): #TODO Test new danger logic
        if self.player.pos in self.map.super_pellets_positions:
            print("YOU DANGEH!")
            self.ticker.start_you_are_dangerous_ticker()
        if self.enemy.pos in self.map.super_pellets_left:
            print("HE DANGEH!")
    ...
    def get_move(self): # AI LOGIC GOES HERE!
        if self.states.is_status_quo():
            ...
            return move
        elif self.states.you_are_dangerous(): #A "you dangeh" print should always directly preceed a:
            print("HUNT!!")
            path = self.map.get_bf_path(self.player.pos,pos_goal=self.enemy.pos)
            first_move_pos = path[1]
            move = self.map.get_move_from(self.player.pos,first_move_pos)
            return move
        elif self.states.enemy_is_dangerous(): #A "he dangeh" print should always directly preceed a:
            print("OH NOES!? D:")
